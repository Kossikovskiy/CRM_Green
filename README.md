...
Это не ошибка сервера — это конфликт **ветки PR** с обновлённым `main`.

Чтобы не решать вручную, используйте один скрипт (из feature-ветки):
...
Что делает `sync_pr.sh`:
1. подтягивает `origin/main`;
2. сливает его в вашу ветку c `-X ours`;
3. если остаются сложные конфликты — автоматически делает `checkout --ours` и коммит (то есть всегда оставляет **ваши новые версии файлов** в конфликтных местах);
4. пушит ветку в GitHub.

После этого PR обычно становится mergeable без ручного редактирования конфликтов.

> Если PR отмечен как Draft, отдельно нажмите **Ready for review** — иначе GitHub не даст merge даже без конфликтов.
